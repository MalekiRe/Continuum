...
pub enum Function {
    Native {
        name: String,
        arity: u32,
        func: fn(&mut crate::kernel::Kernel, &mut crate::vm::env::EnvRef, Vec<Value>) -> Result<Value, String>,
    },
    Interpreted {
        params: Vec<String>,
        body: Vec<Value>,
        ...
        match type_ {
            "native" => {
                ...
                Ok(Function::Native {
                    name,
                    arity,
                    func: |_, _, _| Err("native function not available after deserialization; re-register it".into()),
                })
            }
            "interpreted" => {
